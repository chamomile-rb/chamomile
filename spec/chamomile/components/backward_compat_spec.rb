...
RSpec.describe "backward compatibility" do
  ...
  describe "Viewport setters" do
    it "set_width still works" do
      vp = Chamomile::Viewport.new(width: 80, height: 24)
      vp.set_width(100)
  ...
  describe "Viewport#update still works" do
    it "update is an alias for handle" do
      vp = Chamomile::Viewport.new(width: 80, height: 3)
  ...
  describe "Table::Column.new still works" do
    it "creates columns with the old struct form" do
      columns = [
        Chamomile::Table::Column.new(title: "Name", width: 20),
        Chamomile::Table::Column.new(title: "Size", width: 10),
      ]
      table = Chamomile::Table.new(columns: columns, rows: [%w[foo 10]])
  ...
  describe "Table#update still works" do
    it "update is an alias for handle" do
      table = Chamomile::Table.new(
        columns: [Chamomile::Table::Column.new(title: "Name", width: 20)],
        rows: [["a"], ["b"], ["c"]]
      )
      table.focus
      msg = Chamomile::KeyEvent.new(key: "j", mod: [])
