...
RSpec.describe "Chamomile::List::DEFAULT_KEY_MAP" do
  let(:key_map) { Chamomile::List::DEFAULT_KEY_MAP }
  ...
  it "maps j to cursor_down" do
    ...
    expect(Chamomile::KeyBinding.key_matches?(msg, key_map, :cursor_down)).to be true
  end

  it "maps / to filter" do
    msg = Chamomile::KeyMsg.new(key: "/", mod: [])
    expect(Chamomile::KeyBinding.key_matches?(msg, key_map, :filter)).to be true
  end

  it "maps escape to clear_filter" do
    msg = Chamomile::KeyMsg.new(key: :escape, mod: [])
    expect(Chamomile::KeyBinding.key_matches?(msg, key_map, :clear_filter)).to be true
  end
end
