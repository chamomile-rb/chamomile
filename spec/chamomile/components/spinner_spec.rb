...
RSpec.describe Chamomile::Spinner do
  describe "initialization" do
    it "defaults to LINE spinner type" do
      spinner = described_class.new
      expect(spinner.spinner_type).to eq(Chamomile::Spinners::LINE)
    end

    it "accepts a custom type" do
      spinner = described_class.new(type: Chamomile::Spinners::DOT)
      expect(spinner.spinner_type).to eq(Chamomile::Spinners::DOT)
    end
  ...
  describe "#view" do
    it "returns the current frame" do
      spinner = described_class.new(type: Chamomile::Spinners::ELLIPSIS)
      expect(spinner.view).to eq("")
    end
  end
  ...
  describe "#tick_cmd" do
    ...
    it "produces a SpinnerTickMsg with matching id" do
      ...
      expect(msg).to be_a(Chamomile::SpinnerTickMsg)
  ...
  describe "#update" do
    ...
    it "ignores SpinnerTickMsg with wrong id" do
      spinner = described_class.new
      msg = Chamomile::SpinnerTickMsg.new(id: -999, tag: 0, time: Time.now)
      cmd = spinner.update(msg)
      expect(cmd).to be_nil
      expect(spinner.view).to eq("|")
    end
  ...
  describe "#spinner_type=" do
    it "changes the spinner type" do
      spinner = described_class.new
      spinner.spinner_type = Chamomile::Spinners::DOT
      expect(spinner.spinner_type).to eq(Chamomile::Spinners::DOT)
      expect(spinner.view).to eq("⣾")
    end
